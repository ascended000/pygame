...
display.set_caption("level display")
clock=time.Clock()
...
for i in range(5):
    Block("asset_level/little_bush.png", 60, 30)
for i in range(5):
    Block("asset_level/little_leaf.png", 60, 60)
for i in range(5):
    Block("asset_level/sprout_2.png", 60, 40)
for i in range(5):
        ...
        if e.type == QUIT:
            ...
            for block in blocks:
                if block.rect.collidepoint(x,y):
                    select_block = block
                    offset_x = block.rect.x - x
                    offset_y = block.rect.y - y
                    break
        elif e.type == MOUSEBUTTONUP:
            select_block = None
        elif e.type == MOUSEMOTION and select_block:
            x, y = e.pos
            select_block.rect.x = x - offset_x
            select_block.rect.y = y - offset_y
    window.fill((0,0,0))
    for block in blocks:
        block.reset()
    display.update()
